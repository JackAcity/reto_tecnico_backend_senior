...
El check `dependency-audit` solo analiza PRs que modifican un manifest, lockfile o
la configuración de Dependabot. Restaura el grafo NuGet y ejecuta `npm audit`; el
gate falla ante una vulnerabilidad conocida de severidad alta o crítica. No interpreta
la ausencia de un hallazgo como ausencia de riesgo.
...
- [dependency-review.yml](../../.github/workflows/dependency-review.yml) ejecuta
  `dotnet list --vulnerable --include-transitive` con una validación explícita del
  reporte JSON y `npm audit --audit-level=high`. Usa acciones fijadas a SHA completo
  y token de solo lectura; esto preserva CTL-006 y CTL-007.
- La auditoría de imágenes Docker y de licencias no se simula aquí: requiere sus
  propias políticas y herramientas en gates posteriores. Los cambios de Compose y
  GitHub Actions sí quedan bajo actualización semanal de Dependabot y revisión de PR.
...
resultado de `dependency-audit`, alerta o advisory, decisión de actualización y
