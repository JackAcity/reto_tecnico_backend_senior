    ...
    ILogger<ConsumidorCargaMasiva> log) : BackgroundService
{
    ...
    private async Task ProcesarEntregaAsync(BasicDeliverEventArgs ea, CancellationToken ctHost)
    {
        ...
        {
            var intentos = ContarIntentosPrevios(ea.BasicProperties.Headers);
            ...
            if (intentos + 1 >= MaxIntentos)
            {
                ...
                await canal.BasicPublishAsync(
                    Topologia.Exchange, Topologia.RkMuerto, mandatory: false,
    ...
    /// mensaje muere hacia un dead-letter-exchange. Se busca la entrada de ESTA cola
    /// y se lee su "count" — es la forma estándar de contar reintentos sin mantener
    /// estado propio ni un plugin adicional.
    /// </summary>
    /// <summary>Pública para poder probarla directo, sin levantar RabbitMQ.</summary>
    public static int ContarIntentosPrevios(IDictionary<string, object?>? headers)
    {
        ...
        return muertes
            ...
            .Where(m => m.TryGetValue("queue", out var q) && TextoDe(q) == Topologia.ColaCarga)
