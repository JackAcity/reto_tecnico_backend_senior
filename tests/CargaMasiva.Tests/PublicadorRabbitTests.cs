...
public sealed class PublicadorRabbitTests : IAsyncLifetime
{
    ...
    /// se pierde en silencio — PublicarAsync lanza PublishReturnException. Esto
    /// es lo que hace que el try/catch de ServicioCargas.RegistrarAsync (§C7)
    ...
    {
        var ex = await Assert.ThrowsAsync<PublishReturnException>(() =>
            _publicador.PublicarAsync("ruta.que.no.existe.en.ningun.binding", new { x = 1 }, "test-return"));

        Assert.Contains("ruta.que.no.existe.en.ningun.binding", ex.Message);
    }
    ...
    {
        try { await _publicador.PublicarAsync("otra.ruta.sin.binding", new { x = 1 }, "test-return"); }
        catch (PublishReturnException) { /* ya cubierto por el otro test */ }
